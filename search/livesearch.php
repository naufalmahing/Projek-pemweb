    ...
    for($i=0; $i<($x->length); $i++) {
        ...
        if ($y->item(0)->nodeType==1 && $z->item(0)->nodeType==1) {
            if (stristr($y->item(0)->childNodes->item(0)->nodeValue,$q) || stristr($z->item(0)->childNodes->item(0)->nodeValue, $q)) {
                if ($hint=="") {
                    $hint="<a class='url' href='../reader/detail.php?surah=" . $y->item(0)->nodeValue . "&nama_surah=" . $z->item(0)->nodeValue . "'>" . $z->item(0)->nodeValue . "</a>";
                } else {
                    $hint=$hint . "<br /><a class='url' href='../reader/detail.php?surah=" . $y->item(0)->nodeValue . "&nama_surah=" . $z->item(0)->nodeValue . "'>" . $z->item(0)->nodeValue . "</a>";
                }
            }
        }
    }
